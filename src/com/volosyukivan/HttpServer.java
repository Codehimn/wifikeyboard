...
public abstract class HttpServer extends Thread {
  ...
  /**
   * Invoke from network thread and execute action on main thread
   * (synchronized).
   * 
   * @param action
          ...
          if (key == serverkey) {
            ...
            try {
              ...
              if (key.isReadable()) {
                ...
                // Log.d("wifikeyboard", "finished read event, allocs = " +
                // size);
              ...
              if (newState == prevState)
                continue;
              key.interestOps((newState == HttpConnection.ConnectionState.SELECTOR_WAIT_FOR_NEW_INPUT ? SelectionKey.OP_READ
                  : 0)
                  | (newState == HttpConnection.ConnectionState.SELECTOR_WAIT_FOR_OUTPUT_BUFFER ? SelectionKey.OP_WRITE
                      : 0));
